...
// 创建一个新的钱包
func (s *WalletService) createWallet() (string, string) {
	// 生成一个新的私钥
...
// 获取指定地址的余额
func (s *WalletService) getBalance(address string) *big.Float {
	// 连接到Sepolia测试网
	...
	// 从以太坊客户端获取指定地址的余额
	// context.Background()是一个空的上下文，上下文在 Go 中用于控制并 发操作的生命周期，例如取消操作或传递请求范围的数据。
	// nil表示查询最新的区块
	balance, err := client.BalanceAt(context.Background(), addr, nil)
	if err != nil {
		log.Fatal(err)
	}
	// 将余额从Wei转换为Ether，Wei是以太坊中最小的单位，而Ether是以太 坊的主要货币单位。1 Ether等于10^18 Wei。
...
// 将一个账户中的ETH转账到另一个账户
func (s *WalletService) transfer(fromAddress string, toAddress string) {
	// 连接到Sepolia测试网
	client, err := ethclient.Dial("https://sepolia.infura.io/v3/766c230ed91a48a097e2739b966bbbf7")
	if err != nil {
		log.Fatal(err)
	...
	// nonce 是一个用于标识以太坊账户交易顺序的计数器。每当账户发起一
	// PendingNonceAt 方法用于获取指定地址的下一个待处理交易的 nonce
	...
	// SignTx 调用crypto.Sign方法生成签名，并将签名附加到交易上并返回 一个已签名的交易实例。
	// NewEIP155Signer 方法用于创建一个新的 EIP-155 签名者实例，并设置链ID。
	// EIP-155 是以太坊改进提案 155，它引入了一种新的交易签名方法，通 过在签名中包含 chainID 来防止跨链重放攻击。
	// chainID 是以太坊网络的链标识符，不同的以太坊网络（例如主网、测 试网）有不同的 chainID。在签名交易时，chainID 被用来确保交易只能在特定的网络上有效，从而防止跨链重放攻击。
